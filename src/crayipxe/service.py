...
def create_binaries(fname, script, cert=None, arch='x86_64', kind='efi', bearer_token=None, ipxe_build_debug=False):
    """
    Creates a new ipxe binary and registers it to the TFTP service for
    consumption.
    """
    print('create_binaries called for %s, %s' % (kind, arch))
    ...
    try:
        ...
        if ipxe_build_debug:
            build_command.append('DEBUG=httpcore:2')
    except KeyError:
    ...
    if bearer_token:
        print('Compiling with BEARER_TOKEN=%s' % (bearer_token))
        build_command.append('BEARER_TOKEN=%s' % (bearer_token))

    # Build it out
    print("Running command: %s" % (build_command))
    ...
    print("Newly created ipxe binary created: '%s'" % (os.path.join(TFTP_MOUNT_DIR, fname)))
...
def main():
    ...
    try:
        ...
        print("FAILED attempting to copy {} to {}".format(uri_file, copy_file))
        raise
    ...
    print("Monitoring associated configmaps for related changes...")
    while not run_context.kill_now:
        ...
        if any([settings_changed, ca_public_key_changed, bss_script_changed,
            ...
            create_binaries('ipxe.efi', bss_script, cert=public_cert,
        ...
        if any([settings_changed, ca_public_key_changed, shell_script_changed,
            ...
            create_binaries('debug.efi', shell_script, cert=public_cert,
                            bearer_token=bearer_token,
                            ipxe_build_debug=ipxe_build_debug)
...
if __name__ == '__main__':
    ...
    print("Cray IPXE builder Initializing...")
